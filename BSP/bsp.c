...
{
	// 配置LED引脚       PC4
	GPIO_Init(GPIOB, GPIO_Pin_3, GPIO_Mode_Out_PP_High_Fast);          // 初始化为低电平    点亮LED 
...
void ADC_Initial(void)
{
    ...
    GPIO_Init(GPIOB, GPIO_Pin_4, GPIO_Mode_In_FL_No_IT);     // 设置PA->4 为悬空输入，并中断禁止
    ADC_Init(ADC1,
...
uint16_t ADC_channel_change1(void)
{
    ADC_Initial();
    return ADC_Data_Read();
}

//uint16_t ADC_Channel_change2(void)
//{
//    ADC_Init2();
//    return ADC_Data_Read2();
//}
//
//void ADC_Init2(void)
//{
//	ADC_DeInit(ADC1);
//     CLK_PeripheralClockConfig(CLK_Peripheral_ADC1, ENABLE);  // 使能ADC1时钟
//    GPIO_Init(GPIOB, GPIO_Pin_3, GPIO_Mode_In_FL_No_IT);     // 设置PA->4 为悬空输入，并中断禁止
//    ADC_Init(ADC1,
//             ADC_ConversionMode_Single,   // 单次转换模式
//             ADC_Resolution_10Bit,        // 12位精度转换
//             ADC_Prescaler_2              // 时钟设置为2分频
...
//
//    ADC_ChannelCmd(ADC1,
//                   ADC_Channel_15,         // 设置为通道2进行采样
//                   ENABLE);
//
//    ADC_Cmd(ADC1 , ENABLE);               // 使能ADC  
//    
//    ADC_Data_Read2();                      // 预先读取两次误差较大的值
//    ADC_Data_Read2(); 
//}
//
//uint16_t ADC_Data_Read2(void)
//{
//    ADC_SoftwareStartConv(ADC1);      //启动ADC
//
//    while(ADC_GetFlagStatus(ADC1, ADC_FLAG_EOC) == 0);   // 等待转换结束
