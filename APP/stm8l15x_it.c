...
INTERRUPT_HANDLER(USART1_RX_IRQHandler, 28)
{
  ...
  */ 
    ...
    {     
		...
		//NBiotRcvTemp[NBiotRcvCnt++] = USART_ReceiveData8(USART1);
		Usart_DataTemp = USART_ReceiveData8(USART1);
		NBiotRcvTemp[NBiotRcvCnt++] = Usart_DataTemp;
		Recieve_NB_Mesag(Usart_DataTemp);

		Recieve_CGATT_Mesag(Usart_DataTemp);

		if (NB_GetSigCSQ_Flag == 1)
		{
			
			NBiot_GetSigCSQ[NB_GetSig_Cnt++] = Usart_DataTemp;
			if (NB_GetSig_Cnt > 34)
			{
				NB_GetSig_Cnt = 0;
			}
			if (Usart_DataTemp == 0x2c)   //0x2c  “，”
			{
				NB_GetSigCSQ_Flag = 0;
			}
		}
